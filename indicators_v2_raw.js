'use strict';
/**
 * indicators_v2.js — tape-reading additions (daily-bar proxies for order flow).
 * Pure functions, null-tolerant. Merge into lib/indicators.js exports or require alongside.
 * All take `quotes` = [{date, open, high, low, close, volume}] oldest→newest.
 */
const { getSMA, getATR, getEMA } = require('./indicators.js');
...
// ── Volume / accumulation tape proxies ──────────────────────────────────────
...
function obvSlope(quotes, n = 20) {
  ...
  return +(((s[s.length - 1] - s[0]) / n) / avgVol).toFixed(3); // OBV change per day in avg-vol units
}

// ── Relative strength (the missing alpha filter) ────────────────────────────
...
function relStrength(closes, benchCloses) {
  ...
  return w > 0 ? +(100 * rs / w).toFixed(2) : null; // weighted excess return, in %
}
...
// ── Contraction sequence (VCP proxy) ────────────────────────────────────────
