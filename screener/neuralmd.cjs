#!/usr/bin/env node
/**
 * neuralmd.cjs — Injects AI Neural Hunt markdown/text into the screener dashboard.
 */
const fs = require('fs');

const args = process.argv.slice(2);
if (!args.length) { console.error('Usage: node neuralmd.cjs neural_insights.txt'); process.exit(1); }

const txtFile = args[0];
if (!fs.existsSync(txtFile)) { console.error(`File not found: ${txtFile}`); process.exit(1); }
if (!fs.existsSync('screener_report.html')) { console.error('screener_report.html not found. Run screener.cjs first.'); process.exit(1); }
