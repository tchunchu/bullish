...
function classifySetup(quotes, opts = {}) {
  ...
  // ---- stop construction: floor ≥1×ATR (noise protection), cap ≤maxStopPct (risk control) ----
  const swingLow = Math.min(...quotes.slice(-10).map(q => q.low));
  let stop = Math.max(swingLow, price - 2 * atr);          // tighter of the two valid stops
  stop = Math.min(stop, price - atr);                      // FLOOR: never tighter than 1 ATR (kills noise stops + fantasy R:R)
  stop = Math.max(stop, price * (1 - maxStopPct));         // CAP: never risk > maxStopPct (wins if ATR% > cap)
